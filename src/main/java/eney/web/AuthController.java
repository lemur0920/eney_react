...
//	@RequestMapping(value="/form.do",  method = RequestMethod.POST)
    ...
    public ModelAndView joinAction(BindingResult result, HttpSession session, HttpServletRequest request, @RequestBody UserVO userVO) throws Throwable {
        UserCertifyVo userCertify = (UserCertifyVo) session.getAttribute("userCertify");
        ModelAndView mav = new ModelAndView();
        ...
        if(! UserVO.MEMBER_KIND_CORPORATE.equals(userVO.getMember_kind())){
            ...
            if(userCertify == null){
                ...
                logger.warn("[회원가입] 경고 - 본인 인증 없이 가입 시도 (UserVO:{}, IP: {}", userVO, request.getRemoteAddr());
                joinFormView(userVO, session, request);
            }
        }

        joinValidator.validate(userVO, result);
        if(result.hasErrors()) {
            System.out.println("여기4");
            logger.debug("CustomLog:::검증실패");
            logger.debug(userVO.toString());
            joinFormView(userVO, session, request);
            return mav;
        } else {
            System.out.println("여기5");
            logger.debug("CustomLog:::검증성");
            logger.debug(userVO.toString());
        }
        joinService.insertJoinerInfo(userVO);

        mav.setViewName("redirect:complete.do");
        return mav;
    }
    /**
     * 회원가입 성공 페이지
    ...
    public @ResponseBody Boolean duplicationCheck(@ModelAttribute UserVO userVO, BindingResult result) throws Throwable{
        joinValidator.overlappedUserid(userVO, result);
